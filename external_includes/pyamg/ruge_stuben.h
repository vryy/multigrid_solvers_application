 ...
 *
 */
template<class IndexType, class IndexContainerType, class ValueType, class ValueContainerType>
void classical_strength_of_connection(const IndexType n_row,
                                      const ValueType theta,
                                      const IndexContainerType& Ap, const IndexContainerType& Aj, const ValueContainerType& Ax,
                                      IndexContainerType& Sp, IndexContainerType& Sj, ValueContainerType& Sx)
{
    IndexType nnz = 0;
    Sp[0] = 0;

    for(IndexType i = 0; i < n_row; i++){
        ValueType max_offdiagonal = std::numeric_limits<ValueType>::min();
        ...
        ValueType threshold = theta*max_offdiagonal;
        for(IndexType jj = row_start; jj < row_end; jj++){
            ValueType norm_jj = mynorm(Ax[jj]);
 ...
 *    
 */
template<class IndexType, class IndexContainerType, class IndexVectorType>
void rs_cf_splitting(const IndexType n_nodes,
                     const IndexContainerType& Sp, const IndexContainerType& Sj, 
                     const IndexContainerType& Tp, const IndexContainerType& Tj, 
                           IndexVectorType& splitting)
{
    std::vector<IndexType> lambda(n_nodes,0);
    ...
    std::vector<IndexType> interval_ptr(n_nodes+1,0);
    std::vector<IndexType> interval_count(n_nodes+1,0);
    std::vector<IndexType> index_to_node(n_nodes);
    std::vector<IndexType> node_to_index(n_nodes);

    for(IndexType i = 0; i < n_nodes; i++){
        interval_count[lambda[i]]++;
    }
    for(IndexType i = 0, cumsum = 0; i < n_nodes; i++){
        interval_ptr[i] = cumsum;
        cumsum += interval_count[i];
        interval_count[i] = 0;
    }
    for(IndexType i = 0; i < n_nodes; i++){
        IndexType lambda_i = lambda[i];
        IndexType index    = interval_ptr[lambda_i] + interval_count[lambda_i];
...
//    std::fill(splitting, splitting + n_nodes, U_NODE);
    std::fill(splitting.begin(), splitting.end(), U_NODE);
    ...
    for(IndexType top_index = n_nodes - 1; top_index != -1; top_index--){
        IndexType i        = index_to_node[top_index];
        IndexType lambda_i = lambda[i];
        ...
        else 
        {
            ...
            for(IndexType jj = Tp[i]; jj < Tp[i+1]; jj++){
                IndexType j = Tj[jj];
                ...
                if(splitting[j] == U_NODE){
                    ...
                    //For each k in S_j /\ U
                    for(IndexType kk = Sp[j]; kk < Sp[j+1]; kk++){
                        IndexType k = Sj[kk];
                        ...
                        if(splitting[k] == U_NODE){	      
                            ...
                            IndexType lambda_k = lambda[k];
                            IndexType old_pos  = node_to_index[k];
                            IndexType new_pos  = interval_ptr[lambda_k] + interval_count[lambda_k] - 1;
            ...
            //For each j in S_i /\ U
            for(IndexType jj = Sp[i]; jj < Sp[i+1]; jj++){
                IndexType j = Sj[jj];
                if(splitting[j] == U_NODE){            //decrement lambda for node j
                    ...
                    IndexType lambda_j = lambda[j];
                    IndexType old_pos  = node_to_index[j];
                    IndexType new_pos  = interval_ptr[lambda_j]; 
...
template<class IndexType, class IndexContainerType, class IndexVectorType, class IntegerType = int>
void cljp_naive_splitting(const IndexType n,
                          const IndexContainerType& Sp, const IndexContainerType& Sj,
                          const IndexContainerType& Tp, const IndexContainerType& Tj, 
                          IndexVectorType& splitting,
                          const IntegerType colorflag)
{
  // initialize sizes
  int ncolors;
  IndexType unassigned = n;
  IndexType nD;
  int nnz = Sp[n];
  ...
  std::vector<IndexType> D(n,0);      // marked nodes  in the ind set
  std::vector<IndexType> Dlist(n,0);      // marked nodes  in the ind set
//  std::fill(splitting, splitting + n, U_NODE);
  std::fill(splitting.begin(), splitting.begin() + n, U_NODE);
...
//    vertex_coloring_mis(n, Sp, Sj, &coloring[0]);
    vertex_coloring_mis(n, Sp, Sj, coloring);
  ...
  while(unassigned > 0){
    ...
    for(IndexType iD=0; iD < nD; iD++){
      IndexType c = Dlist[iD];
      for(IndexType jj = Sp[c]; jj < Sp[c+1]; jj++){
        IndexType j = Sj[jj];
        // c <---j
    ...
    for(IndexType iD=0; iD < nD; iD++){
      IndexType c = Dlist[iD];
      for(IndexType jj = Tp[c]; jj < Tp[c+1]; jj++){
        IndexType j = Tj[jj];
 ...
 */      
template<class IndexType, class IndexContainerType, class IndexVectorType>
void rs_direct_interpolation_pass1(const IndexType n_nodes,
                                   const IndexContainerType& Sp, const IndexContainerType& Sj,
                                   const IndexVectorType& splitting,
                                         IndexContainerType& Bp)
{
    IndexType nnz = 0;
    Bp[0] = 0;
    for(IndexType i = 0; i < n_nodes; i++){
...
template<class IndexType, class IndexContainerType, class IndexVectorType, class ValueContainerType, class ValueType = double>
void rs_direct_interpolation_pass2(const IndexType n_nodes,
                                   const IndexContainerType& Ap, const IndexContainerType& Aj, const ValueContainerType& Ax,
                                   const IndexContainerType& Sp, const IndexContainerType& Sj, const ValueContainerType& Sx,
                                   const IndexVectorType& splitting,
                                   const IndexContainerType& Bp, IndexContainerType& Bj, ValueContainerType& Bx)
{
    ...
    for(IndexType i = 0; i < n_nodes; i++){
        if(splitting[i] == C_NODE){
            ...
            ValueType sum_strong_pos = 0, sum_strong_neg = 0;
            for(IndexType jj = Sp[i]; jj < Sp[i+1]; jj++){
                if (splitting[Sj[jj]] == C_NODE){
                    if (Sx[jj] < 0)
            ...
            ValueType sum_all_pos = 0, sum_all_neg = 0;
            ValueType diag = 0;
            ...
            ValueType alpha = sum_all_neg / sum_strong_neg;
            ValueType beta  = sum_all_pos / sum_strong_pos;
            ...
            ValueType neg_coeff = -alpha/diag;
            ValueType pos_coeff = -beta/diag;
    ...
    std::vector<IndexType> map(n_nodes);
    for(IndexType i = 0, sum = 0; i < n_nodes; i++){
        map[i]  = sum;
        sum    += splitting[i];
    }
    for(IndexType i = 0; i < Bp[n_nodes]; i++){
        Bj[i] = map[Bj[i]];
    }
}
...
void rs_standard_interpolation(const IndexType n_nodes,
                               const IndexType Ap[], const IndexType Aj[], const T Ax[],
                               const IndexType Sp[], const IndexType Sj[], const T Sx[],
                               const IndexType Tp[], const IndexType Tj[], const T Tx[],
                                     IndexType Bp[],       IndexType Bj[],       T Bx[])
{
...
//
//template<class IndexType, class T>
//void rs_interpolation(const IndexType n_nodes,
//        const IndexType Ap[], const IndexType Aj[], const T Ax[],
//        const IndexType Sp[], const IndexType Sj[], const T Sx[],
//        const IndexType Tp[], const IndexType Tj[], const T Tx[],
//        std::vector<IndexType> * Bp, std::vector<IndexType> * Bj, std::vector<T> * Bx){
//
//    std::vector<IndexType> lambda(n_nodes,0);
//
//    //compute lambdas
...
//    // index to node - the node located at a given index
//    // node to index - the index of a given node
//    std::vector<IndexType> interval_ptr(n_nodes,0);
//    std::vector<IndexType> interval_count(n_nodes,0);
//    std::vector<IndexType> index_to_node(n_nodes);
//    std::vector<IndexType> node_to_index(n_nodes);
//
//    for(IndexType i = 0; i < n_nodes; i++){
...
//
//
//            for(IndexType n = 0; n <= top_index; n++){
...
//    for(IndexType i = 0; i < n_nodes; i++){
//        if(NodeSets[i] == F_NODE){
//            IndexType row_start = Sp[i];
//            IndexType row_end   = Sp[i+1];
...
//            for(IndexType jj = Ap[i]; jj < Ap[i+1]; jj++){ d_i += Ax[jj]; }
//            for(IndexType jj = Sp[i]; jj < Sp[i+1]; jj++){ d_i -= Sx[jj]; }
//
...
//            for(IndexType jj = Sp[i]; jj < Sp[i+1]; jj++){ //for j in D^s_i
//                IndexType    j = Sj[jj];
//                T   a_ij = Sx[jj];
//                T   a_jl = 0;
//
...
//
//                //compute sum a_jl
//                for(IndexType ll = Sp[j]; ll < Sp[j+1]; ll++){
//                    if(C_i[Sj[ll]]){
...
//    std::vector<IndexType> cnode_index(n_nodes,-1);
//    IndexType n_cnodes = 0;
//    for(IndexType i = 0; i < n_nodes; i++){
//        if(NodeSets[i] == C_NODE){
...
//    for(typename std::vector<IndexType>::iterator iter = Bj->begin(); iter != Bj->end(); iter++){
