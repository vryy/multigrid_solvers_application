...
void symmetric_strength_of_connection(const IndexType n_row,
                                      ...
                                      IndexContainerType& Sp, IndexContainerType& Sj, ValueContainerType& Sx)
{
    ...
    std::vector<ValueType> diags(n_row);
    ...
    for(IndexType i = 0; i < n_row; i++){
        ...
        ValueType eps_Aii = theta*theta*diags[i];

        for(IndexType jj = Ap[i]; jj < Ap[i+1]; jj++){
            const IndexType   j = Aj[jj];
            const ValueType Aij = Ax[jj];
