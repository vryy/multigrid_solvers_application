 ...
 * 
 */
template <class IndexType, class IndexContainerType, class ValueContainerType>
void csr_tocsc(const IndexType n_row,
	           const IndexType n_col, 
	           const IndexContainerType& Ap, 
	           const IndexContainerType& Aj, 
	           const ValueContainerType& Ax,
	                  IndexContainerType& Bp,
	                  IndexContainerType& Bi,
	                  ValueContainerType& Bx)
{  
    const IndexType nnz = Ap[n_row];
...
//    std::fill(Bp, Bp + n_col, 0);
    ...
    for(IndexType col = 0, cumsum = 0; col < n_col; col++){     
        IndexType temp  = Bp[col];
        Bp[col] = cumsum;
        cumsum += temp;
    }
    Bp[n_col] = nnz; 

    for(IndexType row = 0; row < n_row; row++){
        for(IndexType jj = Ap[row]; jj < Ap[row+1]; jj++){
            IndexType col  = Aj[jj];
            IndexType dest = Bp[col];
    ...
    for(IndexType col = 0, last = 0; col <= n_col; col++){
        IndexType temp  = Bp[col];
        Bp[col] = last;
        last    = temp;
    }
 ...
 *   The value of nnz(C) will be stored in Cp[n_row] after the first pass.
 *
 * Note: 
 ...
 *
 */
template <class IndexType, class IndexContainerType, class IntegerType = int>
void csr_matmat_pass1(const IndexType n_row,
                      const IndexType n_col, 
                      const IndexContainerType Ap, 
                      const IndexContainerType Aj, 
                      const IndexContainerType Bp,
                      const IndexContainerType Bj,
                            IndexContainerType& Cp)
{
    // method that uses O(n) temp storage
    std::vector<IntegerType> mask(n_col, -1);
    Cp[0] = 0;
    ...
    for(IndexType i = 0; i < n_row; i++){
        for(IndexType jj = Ap[i]; jj < Ap[i+1]; jj++){
            IndexType j = Aj[jj];
            for(IndexType kk = Bp[j]; kk < Bp[j+1]; kk++){
                IndexType k = Bj[kk];
                if(mask[k] != i){
                    mask[k] = i;                        
                    nnz++;
 ...
 *
 */
template <class IndexType, class IndexContainerType, class ValueType = double, class ValueContainerType, class IntegerType = int>
void csr_matmat_pass2(const IndexType n_row,
      	              const IndexType n_col, 
      	              const IndexContainerType Ap, 
      	              const IndexContainerType Aj, 
      	              const ValueContainerType Ax,
      	              const IndexContainerType Bp,
      	              const IndexContainerType Bj,
      	              const ValueContainerType Bx,
      	                    IndexContainerType& Cp,
      	                    IndexContainerType& Cj,
      	                    ValueContainerType& Cx)
{
    std::vector<IntegerType> next(n_col,-1);
    std::vector<ValueType> sums(n_col, 0);
    ...
    for(IndexType i = 0; i < n_row; i++){
        ...
        IndexType jj_start = Ap[i];
        IndexType jj_end   = Ap[i+1];
        for(IndexType jj = jj_start; jj < jj_end; jj++){
            IndexType j = Aj[jj];
            ValueType v = Ax[jj];

            IndexType kk_start = Bp[j];
            IndexType kk_end   = Bp[j+1];
